...
namespace CabInvoiceGenerator
{
    public class UserRideData
    {
        ...
        public void AddUserToDIctionary(int id, Ride[] rides, CabInvoice.RideType type)
        {
            int flag = 0;
            List<string> summaryList = new List<string>();
            string rideType = string.Empty;
            CabInvoice cabInvoice = new CabInvoice(type);
            //Check the type of ride
            if (type.Equals(CabInvoice.RideType.NORMAL))
            {
                rideType = "Normal";
            }
            else
